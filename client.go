...
func (c *Client) TrackThroughProxy(proxyURL string, keywords ...string) (*Connection, error) {
	form := url.Values{"track": {strings.Join(keywords, ",")}}
	return c.filter(form, proxyURL)
}
...
func (c *Client) filter(form url.Values, proxyURL string) (*Connection, error) {
	...
	conn := c.buildConnection(proxyURL)
...
func (c *Client) buildConnection(proxyURL string) *Connection {
	if proxyURL == "" {
		return newConnection(c.Timeout, "")
	} else {
		return newConnection(c.Timeout, proxyURL)
	}
}
